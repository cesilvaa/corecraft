...
RSpec.describe MempoolSummary do
  ...
  context "when mempool is empty" do
    before { allow(rpc).to receive(:call).with("getrawmempool", [true]).and_return({}) }
  ...
  context "with transactions" do
    ...
    before { allow(rpc).to receive(:call).with("getrawmempool", [true]).and_return(raw_mempool) }
  ...
  describe "robustness" do
    it "ignores transactions without valid size" do
      ...
      allow(rpc).to receive(:call).with("getrawmempool", [true]).and_return(txs)
      expect(described_class.new(rpc).call[:tx_count]).to eq(1)
    end
  end
end
